...
def summarize_results() -> list[tuple[str, int, float, float, float]]:
    """Summarize the results per configuration tested."""
    db = sqlite3.connect(DATABASE_FILE)
    query = f"""
    -- sql
    SELECT 
    CASE
        WHEN inner_query.collation_type = 'ICU' AND inner_query.ICU_FROZEN = 1 AND inner_query.ICU_EXTRA_TAILORING = 1 THEN 'ICU_tailoring'
        WHEN inner_query.collation_type = 'ICU' AND inner_query.ICU_FROZEN = 1 AND inner_query.ICU_EXTRA_TAILORING = 0 THEN 'ICU_frozen'
        WHEN inner_query.collation_type = 'ICU' AND inner_query.ICU_FROZEN = 0 AND inner_query.ICU_EXTRA_TAILORING = 0 THEN 'ICU_locale'
        ELSE 'MySQL'
    END AS config,
	COUNT(*),
